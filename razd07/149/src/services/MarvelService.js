...
class MarvelService {
	...
	getAllCharacters = async () => {
		const res = await this.getResource(`${this._api.baseUrl}characters?limit=9&offset=210&apikey=${this._api.apikey}`);
		return res.data.results.map( this._transformCharacter );
	};

	getCharacter = async (_id) => {
		const res = await this.getResource(`${this._api.baseUrl}characters/${_id}?apikey=${this._api.apikey}`);
